...
def bf_rarevariation(prior):
    gene = f'"{gene_input}"'
    q = "{GeneAssociations52k(gene:" + gene + ") {pValue,phenotype,beta,dataset,gene, masks { mask,beta, pValue, stdErr, combinedAF}}}"
    ...
    if isExomewideSignificant(assoc_52kdata_df):
        ...
        rareBF = rare_variation_strength(prior,beta,stdErr)
...
def cs_body():
    ...
    if isGwasSignificant(assoc_data_df):
        col1.code("* " + f'{gene_input}' +  " is GWAS siginificant" + "\n* " + bf_commonvariation()["Category"][0] + " Evidence" + "\n* Common Variation Bayes Factor = " + str(bf_commonvariation()["Bayes Factor"][1]) )
    else:
        col1.code("* " + f'{gene_input}' +  " is not GWAS siginificant" + "\n* "  + bf_commonvariation()["Category"][0] + " Evidence" + "\n* Common Variation Bayes Factor = " + str(bf_commonvariation()["Bayes Factor"][1]) )
  
    commonvariation_expander.text("*Compelling: HuGe Score >= 350 Extreme: >=100 Very Strong: >=30 Strong: >=10 Moderate: >=3 Anecdotal: >1 No Evidence: <=1")
    

    #Common Evidence
    col2.subheader('Rare Evidence')
    rarevariation_expander = col2.beta_expander("See explaination")
    prior_input = float(rarevariation_expander.text_input("Enter prior",0.3696))
    if isExomewideSignificant(assoc_52kdata_df):
        col2.code("* " + f'{gene_input}' +  " is Exome wide siginificant" + "\n* " + bf_rarevariation(prior_input)["Category"][0] + " Evidence" + "\n* Rare Variation Bayes factor = " + str(bf_rarevariation(prior_input)["Bayes Factor"][1]) )
    else:
        col2.code("* " + f'{gene_input}' +  " is not Exome wide siginificant" + "\n* "  + bf_rarevariation(prior_input)["Category"][0] + " Evidence" + "\n* Rare Variation Bayes factor = " + str(bf_rarevariation(prior_input)["Bayes Factor"][1]) )
  
    rarevariation_expander.code(bf_rarevariation(prior_input))
    rarevariation_expander.text("*Compelling: HuGe Score >= 350 Extreme: >=100 Very Strong: >=30 Strong: >=10 Moderate: >=3 Anecdotal: >1 No Evidence: <=1")
    for index, row in assoc_52kdata_df.iterrows():
        if row['phenotype'] == phenotype_input:
            masks = row['masks']
    ...
    with st.beta_container():
        ...
        st.text("*BF = Bayes Factor")
        st.write(pd.DataFrame({
            'BF for Common variation':[bf_commonvariation()["Bayes Factor"][1]],
            'BF for Rare Variation':[bf_rarevariation(prior_input)["Bayes Factor"][1]],
            'Combined BF':[bf_combinedvariation["Bayes Factor"][1]],
